...
module.exports = {
  ...
  run: async (client, message, args) => {
    try {
      ...
      if (!time) {
        return message.reply({content: "please use the format \`.create [time] [silent | private]\` when creating reminders. Thank you.", isSilent: true})
      }
      ...
      setTimeout(async () => {
        ...
        if (exists) {
          ...
          const embed = new Embed()
            .setColor("#F5C400")
            .setTitle("Time's Up!")
            .setDescription(`<@${userId}> your time is up! You have now been reminded with the following message: \`\`\`${reminder}\`\`\` wanna make another one? do \`.create [time] [message] [private or silent]\``)
